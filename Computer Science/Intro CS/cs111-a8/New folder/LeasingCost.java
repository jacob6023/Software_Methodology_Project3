public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        int s = description.indexOf("type:")+4;
        String a = description.substring(s, description.length());
        int f = a.indexOf(';');
        String t = a.substring(1,f);
        s = description.indexOf("name:")+4;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        String name = a.substring(1,f);
        s = description.indexOf("due:")+3;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        double due=Double.parseDouble(a.substring(1,f));
        s = description.indexOf("length:")+6;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        int length=Integer.parseInt(a.substring(1,f));
        s = description.indexOf("monthly:")+7;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        double monthly=Double.parseDouble(a.substring(1,f));
        s = description.indexOf("mile/unit:")+9;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        double usage=Double.parseDouble(a.substring(1,f));
        s = description.indexOf("allowance:")+9;
        a = description.substring(s, description.length());
        f = a.indexOf(';');
        int milleageAllowence=Integer.parseInt(a.substring(1,f));
        double charger=0;
        Vehicle v;
        Fuel fuel;
        Lease lease;
        if(t.equals("electric")) {
            s = description.indexOf("charger:") + 7; 
            String precharge = description.substring(s, description.length());
            f = precharge.indexOf(';');
            charger = Double.parseDouble(precharge.substring(1,f));
            lease = new Lease(due,length, monthly,milleageAllowence);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        double costgas = 8.887;
        double costk =.453;
        double lcost = 0.0;
        double totalcost = 0.0;
        double costemissions = 0;
        double fuelcost = 0;
        for (int i =0; i< vehicles.length; i++) {
            double numberOfMonth = vehicles[i].getLease().getLeaseLength();
            double usage = vehicles[i].getFuel().getUsage();
            double mileageAllowance = vehicles[i].getLease().getMileageAllowance();
            lcost = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(),vehicles[i].getLease().getLeaseLength(),vehicles[i].getLease().getMonthlyCost());

        if (vehicles[i].getFuel().getType()== 1) {
            costemissions = computeCO2(numberOfMonth, usage, mileageAllowance, costgas);
            fuelcost=computeFuelCost(numberOfMonth, usage, mileageAllowance, gasPrice);
        }

        else { 
            costemissions =computeCO2(numberOfMonth, usage, mileageAllowance, costk);
            fuelcost=computeFuelCost(numberOfMonth, usage, mileageAllowance, electricityPrice);
        }
        vehicles[i].setCO2Emission(costemissions);
        vehicles[i].setFuelCost(fuelcost);
        totalcost = fuelcost + lcost;
        if(vehicles[i].getFuel().getType()==2){
            totalcost+=vehicles[i].getFuel().getCharger();
        }
